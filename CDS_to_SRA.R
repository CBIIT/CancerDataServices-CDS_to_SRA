...
#Cancer Data Services - CDS_to_SRA.R

#This script will take data from a validated CDS submission manifest, and create an SRA submission file specifically for a CDS project.
...
list_of_packages=c("dplyr","tidyr","openxlsx","readr","stringi","readxl","optparse","tools")
...
#A one to one addition of each row from the CDS template to the dbGaP SRA template
SRA_df$sample_ID=df$sample_id
...
cat(paste("\n\nThe dbGaP SRA metadata submission has been created here: ",path,output_file,".xlsx.\n\n",sep = ""))
